...
def main(args):
    ...
    while cnt != 0:
        ...
        if it>= args.t:
            con.execute('CREATE TEMP TABLE tpv AS SELECT d.id,d.dist,d.batch FROM (SELECT t1.*, row_number() over (PARTITION BY batch ORDER BY dist) AS class_rank FROM tp1 t1) d WHERE d.class_rank <= ? order by batch,dist',parameters=[args.w]) 
        else:
        ...
        con.execute('CREATE TEMP TABLE tp2 AS SELECT DISTINCT batch,unnest(nbs) as neighbor FROM tpv JOIN Graph_index_table USING(id)') 
        ...
        con.execute('CREATE TABLE tp_table AS SELECT DISTINCT d.id,d.dist,d.batch FROM (SELECT l1.*, row_number() over (PARTITION BY batch ORDER BY dist) AS class_rank FROM (SELECT * FROM L_table UNION SELECT * FROM tp3) l1) d WHERE d.class_rank <= ? order by batch,dist',parameters=[args.l]) 
